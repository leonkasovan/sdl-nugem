...
void Sffv2::loadSffFile()
{
    ...
    m_nsprites = read_uint32(charfile);
    uint32_t first_palette_offset = read_uint32(charfile);
    m_npalettes = read_uint32(charfile);
...
void Sffv2::outputColoredPixel(uint8_t color, const uint32_t indexPixel, const Sffv2Palette & palette, SDL_Surface * surface, const uint32_t surfaceSize)
{
    if (indexPixel < surfaceSize) {
        ...
        uint8_t * colorarray = m_ldata + (palette.ldataOffset + color * 4);
